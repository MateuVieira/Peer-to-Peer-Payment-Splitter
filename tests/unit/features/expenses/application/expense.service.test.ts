...
import type { IExpenseRepository, CreateExpenseData } from '../../../../../src/features/expenses/domain/expense.repository.js';
...
describe('ExpenseService - Integration Tests', () => {
  ...
  describe('createExpense', () => {
    ...
    it('should successfully create an expense with EQUAL split', async () => {
      ...
      mockExpenseRepository.create.mockImplementation((data: CreateExpenseData): Promise<Expense> => {
      ...
      const actualRepoParticipants = [...createCallArgs.participants].sort((a,b) => a.participantId.localeCompare(b.participantId));
      ...
      const actualResultParticipants = [...result.participants].sort((a,b) => a.participantId.localeCompare(b.participantId));
      expect(actualResultParticipants.map(p => ({participantId: p.participantId, shareAmount: p.shareAmount}))).toEqual(expectedRepoParticipants);
    });
