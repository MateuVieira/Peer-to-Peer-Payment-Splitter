...
import type { SplitStrategyContext, ParticipantShare } from '../../../../../src/features/expenses/domain/splitStrategy.js';
...
describe('EqualSplitStrategy', () => {
  ...
  it('should split an amount equally among all group members', () => {
    ...
    const sortedActualShares = [...actualShares].sort((a, b) => a.participantId.localeCompare(b.participantId));
    const sortedExpectedShares = [...expectedShares].sort((a, b) => a.participantId.localeCompare(b.participantId));
  ...
  it('should throw AppError if group has no members', () => {
    ...
    expect(() => strategy.calculateShares(context)).toThrow(
      new AppError({ httpCode: HttpCode.BAD_REQUEST, description: 'Group has no members for an EQUAL split.' })
    );
  });
  ...
  it('should handle a single member group', () => {
    ...
    const expectedShares: ParticipantShare[] = [
      { participantId: 'user1', shareAmount: 500 },
    ];
    const actualShares = strategy.calculateShares(context);
