...
export const CreateSettlementSchema = z.object({
  ...
  amount: z.number({ required_error: 'Amount is required.' }).int({ message: 'Amount must be an integer (in cents).' }).positive({ message: 'Amount must be a positive number.' }),
  currency: z.string().min(3).max(3).optional().transform(val => val ? val.toUpperCase() : undefined), // Optional, 3-letter code
});
