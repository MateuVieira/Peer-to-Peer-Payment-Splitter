...
export class ProcessService {
  ...
  constructor(strategies: ICommandStrategy[]) {
    this.strategyMap = new Map(strategies.map((strategy) => [strategy.commandType, strategy]));
  }
  ...
  public parseData(row: CSVRecord, jobId: string, userId: string): CommandContext {
    ...
    const isValidCommandType = Object.values(CsvCommandType).includes(
      commandTypeFromRow as CsvCommandType
    );
  ...
  public getStrategy(context: CommandContext): {
