...
export const CreateExpenseParticipantSchema = z.object({
  ...
  shareAmount: z.number().int().min(0, { message: 'Share amount must be a non-negative integer (cents).' }),
});
...
export const CreateExpenseSchema = z.object({
  ...
  currency: z.string().length(3, { message: 'Currency must be a 3-letter code.' }).regex(/^[A-Z]+$/, { message: 'Currency must be uppercase letters.' }).default('USD'),
