...
import { GroupService, CreateGroup, UpdateGroupMember, UpdateGroupDto, CreateGroupSchema, ModifyGroupMemberSchema, UpdateGroupSchema } from '../application/index.js';
...
groupRouter.patch('/:id', validateRequest(UpdateGroupSchema), async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { id: groupId } = req.params;
    if (!groupId) {
      throw new AppError({ httpCode: HttpCode.BAD_REQUEST, description: 'Group ID parameter is required for update.' });
    }
...
groupRouter.delete('/:id', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { id: groupId } = req.params;
    if (!groupId) {
      throw new AppError({ httpCode: HttpCode.BAD_REQUEST, description: 'Group ID parameter is required for deletion.' });
    }
